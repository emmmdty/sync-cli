...
def build_config(*, ssh_config_path: str = "~/.ssh/config", ssh_key_path: str = "~/.ssh/id_ed25519") -> ProjectConfig:
...
def test_status_reports_ssh_config_and_public_key_existence(tmp_path: Path, monkeypatch, capsys) -> None:
    project_dir = tmp_path / "demo"
    project_dir.mkdir()
...
def test_doctor_reports_missing_ssh_config_and_public_key(tmp_path: Path, monkeypatch, capsys) -> None:
    project_dir = tmp_path / "demo"
    project_dir.mkdir()
    ...
    monkeypatch.setattr("sync_remote.cli.load_project_config", lambda cwd=None: (config, project_dir / "sync-remote.yaml"))
    monkeypatch.setattr("sync_remote.cli.resolve_connection_port", lambda config, explicit_port=None: "2222")
    monkeypatch.setattr("sync_remote.cli.shutil.which", lambda name: f"/usr/bin/{name}")
