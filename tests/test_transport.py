...
class FakeCpolarSession:
    ...
    def get(self, url: str, headers=None, timeout: int = 10) -> CpolarFakeResponse:
        if url.endswith("/login"):
            return CpolarFakeResponse(status_code=200, url=url)
        if url.endswith("/status"):
            return CpolarFakeResponse(status_code=200, url=url, text=self.status_html)
        raise AssertionError(f"unexpected GET {url}")

    def post(self, url: str, data=None, headers=None, timeout: int = 10) -> CpolarFakeResponse:
        if url.endswith("/login"):
            return CpolarFakeResponse(status_code=200, url="https://dashboard.cpolar.com/dashboard")
    ...
    class FakeSession:
        ...
        def get(self, url, timeout=10, headers=None):
            ...
            return FakeResponse(
                status_code=200,
                url=url,
                text="<tr><td>gpu</td><td><a>tcp://gpu.internal:2300</a></td></tr>",
            )
